...
- Normal megamenus close after a short mouse-away grace period; while the Publish wizard is running, mouse-away close is disabled, other secondary toolbar megamenus are unavailable, and outside click remains available to close it. After the publish step completes, mouse-away close resumes after a 1000ms grace period and closes immediately if the pointer is outside the megamenu.
...
- Users can clear Publish wizard results after completion without losing the selected scope or publishing target.
